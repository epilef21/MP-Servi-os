...
import {
  db, collection, getDocs, addDoc, updateDoc,
...
// ─── Helpers ──────────────────────────────────────────────────
const SEGURADORAS = ['Mapfre', 'Tempo', 'Maxpar']
...
function fmtDatetime(ts) {
  if (!ts) return '—'
  try { const d = ts.toDate?ts.toDate():new Date(ts); return d.toLocaleString('pt-BR') } catch { return '—' }
}
function fmtSN(v) {
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (!sessionStorage.getItem('mp_auth')) navigate('/login',{replace:true})
  },[navigate])
  ...
  // Financial (admin-only, detail modal)
  ...
  async function loadReports() {
    ...
    try {
      const q = query(collection(db,'checklist'), orderBy('criado_em','desc'))
  ...
  // Sync finForm when a different OS is selected
  useEffect(() => {
    if (!selected) return
    setFinForm({
  ...
  const stats = useMemo(() => {
    const mesReports = filtMes
      ? reports.filter(r => {
          try { const d=r.criado_em?.toDate?.(); return d&&d.toISOString().slice(0,7)===filtMes } catch{return false}
        })
      : reports
    ...
    const hasLucro = mesReports.some(r => getLucro(r)!==null)
    return {
      total:      reports.length,
      hoje:       reports.filter(r => {try{const d=r.criado_em?.toDate?.();return d&&d.toISOString().slice(0,10)===today}catch{return false}}).length,
  ...
  async function changeStatus(id, newStatus) {
    setUpdating(true)
    try {
      await updateDoc(doc(db,'checklist',id),{status:newStatus})
  ...
  // ── OS Form ────────────────────────────────────────────────────
  function setOsField(k,v){ setOsForm(p=>({...p,[k]:v})); if(osErrors[k]) setOsErrors(p=>({...p,[k]:false})) }
  ...
  async function saveOs() {
    ...
    try {
      ...
      const payload = {
        ...
        status:'aguardando_tecnico', origem:'admin', criado_em:serverTimestamp(),
      }
      const ref = await addDoc(collection(db,'checklist'),payload)
      const newRec = {id:ref.id,...payload,criado_em:{toDate:()=>new Date()}}
      setReports(p=>[newRec,...p])
      setShowOsForm(false); setOsForm(OS_INITIAL); setOsErrors({})
  ...
  async function saveFin() {
    ...
    try {
      ...
      await updateDoc(doc(db,'checklist',selected.id), payload)
  ...
  function logout(){ sessionStorage.removeItem('mp_auth'); navigate('/login') }
    ...
    <>
      ...
      <div className="topbar">
        <div className="topbar-left">
          ...
          <div>
            <div className="topbar-title">MP Serviços <span className="topbar-sub">/ Painel Admin</span></div>
          </div>
        </div>
        <div className="topbar-right">
          <button className="btn-new-os" onClick={()=>setShowOsForm(true)}>+ Nova OS</button>
          <Link to="/" className="btn-secondary" style={{textDecoration:'none',fontSize:'.82rem'}}>📋 Formulário</Link>
          <button className="btn-secondary" onClick={loadReports} disabled={loading}>🔄 Atualizar</button>
      ...
      <div className="admin-main">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card green" style={{position:'relative'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}>
              ...
              <input type="month" value={filtMes}
            ...
            {stats.hasLucro ? (
              <>
                <div className={`stat-value ${stats.lucroTotal>=0?'green':'red'}`}>
                  {fmtBRL(stats.lucroTotal)}
                </div>
        ...
        <div className="table-wrap">
          ...
          {!loading&&!error&&filtered.length===0&&<div className="empty-state"><div className="e-icon">📭</div><p>Nenhuma OS encontrada.</p></div>}
              ...
              <tbody>
                {filtered.map(r=>{
                  ...
                  <tr key={r.id} onClick={()=>setSelected(r)}>
                    ...
                    <td data-label="Status"><span className={`badge ${badgeCls(r.status)}`}>{badgeLabel(r.status)}</span></td>
                    <td data-label="Ações" onClick={e=>e.stopPropagation()}>
                      ...
                      {r.status==='aguardando_tecnico'&&(
                        <button className="btn-sm btn-link" onClick={()=>setGeneratedLink({link:buildLink(r),os:r.id,nome:r.nome_segurado,seguradora:r.seguradora,num_assist:r.num_assist})}>🔗</button>
                      )}
          ...
          <div className="modal-box">
            ...
            <div className="modal-body">
              <div className="os-callout">
                📞 Preencha os dados recebidos por telefone. Após salvar, um link pré-preenchido será gerado para enviar ao técnico no WhatsApp.
              </div>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setSelected(null)}>
          <div className="modal-box">
            ...
            <div className="modal-body">
              ...
              {/* Fechamento Financeiro Interno — visível APENAS no painel admin */}
              <div className="md-section" style={{background:'#f0f7f0',border:'1px solid #b8ddb8',borderRadius:8,padding:'14px 16px'}}>
                ...
                <div className="md-grid">
                  <div className="md-field">
                    <label>MO Seguradora (R$)</label>
                    <input type="number" step="0.01" min="0"
                      value={finForm.mo_seguradora}
                      onChange={e=>setFinForm(p=>({...p,mo_seguradora:e.target.value}))}
                      placeholder="0,00"
                  ...
                  <div className="md-field">
                    ...
                    <input type="number" step="0.01" min="0"
                      value={finForm.valor_prestador}
                      onChange={e=>setFinForm(p=>({...p,valor_prestador:e.target.value}))}
                      placeholder="0,00"
                      style={{width:'100%',padding:'6px 8px',border:'1px solid var(--border)',borderRadius:5,fontFamily:'Barlow,sans-serif',fontSize:'.9rem'}}/>
                  </div>
                  <div className="md-field">
                    <label>Valor Deslocamento (R$)</label>
                    <input type="number" step="0.01" min="0"
                      value={finForm.valor_deslocamento}
                      onChange={e=>setFinForm(p=>({...p,valor_deslocamento:e.target.value}))}
                      placeholder="0,00"
                      style={{width:'100%',padding:'6px 8px',border:'1px solid var(--border)',borderRadius:5,fontFamily:'Barlow,sans-serif',fontSize:'.9rem'}}/>
                  </div>
                  <div className="md-field">
